...
public class ConfigurationRepository extends BaseRepository<Config> {
    ...
    public Collection<Config> getDataFromSensor(String sensorId) {
        return (Collection<Config>) em.createQuery("SELECT c FROM Config c WHERE c.id = '"+sensorId+"'").getResultList();
    }
    ...
    public List<Config> normalizeList(List<Config> list) {
        return null;
    }
}
